...
3. SDL2 + OpenGL playback of decoded audio/video streams.
...
* `simple_player` is built when CMake can find SDL2 and OpenGL development packages.
...
`simple_player` decodes the WAV through the IMA ADPCM roundtrip, decodes PNG frames through the simplified MJPEG roundtrip, plays PCM through SDL2, uploads frames into an OpenGL texture, and chooses the displayed frame from the audio/video clock time.
